...
const HeroComponent = () => {
    ...
    const menus: string[] = [
        "Refer", "Benefits", "FAQs", "Support"
    ]

  return (
    <div
    className='w-full max-w-[1360px] mx-auto my-10'>

        <div
        className='w-[800px] mx-auto rounded-full bg-[#1A73E8] bg-opacity-10 flex items-center justify-evenly py-4'>

            {
                menus.map((details, index)=>{
                    return(
                        <div
                        key={index}
                        className={`relative before:absolute before:left-1/2 before:-translate-x-1/2 before:w-2 before:h-2 before:bg-[#1A73E8] before:rounded-full before:-bottom-2 text-2xl transition cursor-pointer ${index === activeMenu ? "text-[#1A73E8] before:opacity-100" : "text-[#4B4B4B] before:opacity-0"}`}
        ...
        <div
        className='w-full shadow-heroComponent bg-[#EEF5FF] overflow-hidden rounded-[29px] flex items-center gap-2 mt-8 relative'>

            <div
            className='w-1/2 flex flex-col px-10 gap-10 z-[1]'>

                <div>

                    <p
                    className='text-[#1A202C] text-[88px] font-extrabold leading-[90px]'>
                    Let’s Learn & Earn
                    </p>

                    <p className='text-[40px] leading-[65px]'>
                        <span>
                        Get a chance to win <br />up-to <span className='text-[54px] font-extrabold text-[#1A73E8]'>Rs. 15,000</span>
                        </span>
                    </p>
